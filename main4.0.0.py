...
class TodoApp:
    ...
    def setup_ui(self):
        ...
        # Canvas for header scrolling
        header_canvas = tk.Canvas(header_container, bg=self.colors['bg'], height=90, highlightthickness=0)
        ...
        title_label = tk.Label(
            header_frame,
            text="📋 PIPELINE",
            font=('Segoe UI', 20, 'bold'),  # Slightly smaller
            bg=self.colors['bg'],
        ...
        self.stash_tab_btn.pack(side=tk.LEFT, padx=5)
        ...
        self.priority_sort_btn.pack(side=tk.LEFT, padx=2)
        ...
        self.date_sort_btn.pack(side=tk.LEFT, padx=2)
    ...
    def refresh_pipeline(self):
        ...
        # Update scroll region with generous padding for deep nesting
        bbox = self.canvas.bbox("all")
        if bbox:
            # Add extra padding especially on the right for horizontal nesting
    ...
    def _render_task_tree(self, task_id: str, x: int, y: int, level: int = 0) -> int:
        ...
        # SWAPPED: Get subtasks (tasks that follow this one horizontally now)
        horizontal_chain = [task_id] + self.get_subtasks(task_id)
        
        max_height = 0
        current_x = x
        
        # Track the maximum x position used by each horizontal item and its workflow siblings
        branch_widths = []
        
        for idx, chain_task_id in enumerate(horizontal_chain):
            ...
            if idx > 0:
                prev_pos = self.task_positions.get(horizontal_chain[idx-1])
                if prev_pos:
                    ...
                    self.canvas.create_line(
                        ...
                        current_x - int(20 * self.card_zoom_level), line_y,
                        fill=self.colors['line'], width=line_width
                    )
                    # Arrow head
                    arrow_size = int(6 * self.card_zoom_level)
                    self.canvas.create_polygon(
                        current_x - int(20 * self.card_zoom_level), line_y - arrow_size,
                        current_x - int(20 * self.card_zoom_level), line_y + arrow_size,
                        current_x - int(10 * self.card_zoom_level), line_y,
            ...
            # SWAPPED: Get workflow siblings (now go vertically below)
            workflow_siblings = self.get_workflow_siblings(chain_task_id)
            
            # Track maximum x used by this branch
            max_branch_x = current_x + self.card_width
            
            if workflow_siblings:
                # Calculate position for workflow siblings (below this card)
                workflow_y = y + card_height + self.card_spacing_v
                workflow_x = current_x + self.level_indent
                
                # Draw vertical line down to workflow siblings
                ...
                # Render all workflow siblings and track their width
                workflow_height = 0
                for wf_id in workflow_siblings:
                    h = self._render_task_tree(wf_id, workflow_x, workflow_y + workflow_height, level + 1)
                    workflow_height += h + int(30 * self.card_zoom_level)
                    
                    # Update max_branch_x based on workflow sibling positions
                    for tid, (tx, ty, tw, th) in self.task_positions.items():
                        if ty >= workflow_y:  # Only consider tasks at or below workflow level
                            max_branch_x = max(max_branch_x, tx + tw)
                
                max_height = max(max_height, card_height + self.card_spacing_v + workflow_height)
            else:
    ...
    def _draw_task_card(self, task_id: str, x: int, y: int) -> int:
        ...
        self.canvas.create_text(
            content_x, content_y,
            text=title, font=('Segoe UI', int(11 * self.card_zoom_level), 'bold'),  # Reduced from 13
            fill='#ffffff' if completed else self.colors['text'],
            anchor='w', width=self.card_width - 2*padding
        )
        content_y += int(24 * self.card_zoom_level)  # Reduced from 30
